...
export function Servicos() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      <h1 style={{ color: "var(--text-h)", fontSize: "1.5rem", margin: 0 }}>Gestão de Serviços</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: "1rem" }}>
        {SERVICOS.map((s) => (
          <div key={s.tipo} style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "8px", padding: "1.5rem", boxShadow: "var(--shadow)" }}>
            ...
            <h2 style={{ color: "var(--text-h)", margin: "0 0 0.5rem", fontSize: "1.25rem" }}>{s.tipo}</h2>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "var(--code-bg)" }}>
              ...
              {SERVICOS.map((s) => (
                <th key={s.tipo} style={{ padding: "1rem", textAlign: "right", color: "var(--text)" }}>{s.tipo}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {[
              ...
              { nivel: "OURO", pontos: "1000+", cor: "#d9b206", bg: "#fef9c3" },
